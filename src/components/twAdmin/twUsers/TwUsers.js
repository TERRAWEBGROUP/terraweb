...
const TwUsers = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = () => {
      ...
      if (adminCookie.length >= 1) {
        ...
        setTimeout(() => {
          fetch("https://api.terraweb.africa/getTWUsers", {
            method: "post",
  ...
  const onDeleteYes = (contactemail, category) => {
    let adminCookie = Cookies.get("sessionidadmin");
    if (adminCookie.length >= 4) {
      fetch("https://api.terraweb.africa/deleteAnyUser", {
        method: "post",
        headers: { "Content-Type": "application/JSON" },
        body: JSON.stringify({
          adminid: adminCookie,
      ...
      customUI: ({ onClose }) => {
        return (
          <div className="bg-white br4 bb b--black-10   pa4 w-100 shadow-5 center ">
            <h1 className="f2 b">Confirm Delete this User</h1>
            <p className="f2">Sure you want to delete this user User?</p>
    ...
    <div className="app-container center br4 mt6 shadow-3">
      ...
      <div className="controlContainer d-grid bg-white  br4 shadow-3 ">
        <label className="grid-item pa3 f3 b">
          <b className="red f2">All Users</b>
